...
if ($subdomain) {
    $pdo = getDB();
    $stmt = $pdo->prepare("SELECT t.*, ts.company_email, ts.company_phone, ts.company_address, ts.company_website FROM tenants t LEFT JOIN tenant_settings ts ON t.id = ts.tenant_id WHERE t.subdomain = ? AND t.status = 'active'");
            ...
            <form id="contactForm" action="/api/contact.php" method="POST" class="bg-white rounded-2xl shadow-lg p-8 md:p-12">
                ...
                <div id="step1" class="step-content">
                    ...
                    <div class="mt-8 text-center">
                        <?php if ($tenant && ($tenant['company_phone'] || $tenant['company_email'] || $tenant['company_website'])): ?>
                        <div class="flex flex-col sm:flex-row items-center justify-center gap-4 text-sm">
                            <?php if ($tenant['company_phone']): ?>
                            <a href="tel:<?= htmlspecialchars($tenant['company_phone'])?>" class="inline-flex items-center text-gray-600 hover:text-gray-900">
                                ...
                                <?= htmlspecialchars($tenant['company_phone'])?>
                            </a>
                            <?php endif; ?>
                            <?php if ($tenant['company_email']): ?>
                            <a href="mailto:<?= htmlspecialchars($tenant['company_email'])?>" class="inline-flex items-center text-gray-600 hover:text-gray-900">
                                <svg class="w-4 h-4 mr-1.5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                                <?= htmlspecialchars($tenant['company_email'])?>
                            </a>
                            <?php endif; ?>
                            <?php if ($tenant['company_website']): ?>
                            <a href="<?= htmlspecialchars($tenant['company_website'])?>" target="_blank" class="inline-flex items-center text-gray-600 hover:text-gray-900">
                                <svg class="w-4 h-4 mr-1.5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"></path></svg>
                                <?= htmlspecialchars(str_replace(['http://', 'https://'], '', $tenant['company_website']))?>
                            </a>
                            <?php endif; ?>
                        </div>
                        <?php else: ?>
