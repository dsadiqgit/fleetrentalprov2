    ...
    <div class="flex-1 flex flex-col overflow-hidden pt-14 lg:pt-0">
        <main class="flex-1 overflow-y-auto bg-gray-50/50 p-4 sm:p-6 lg:p-10">
            ...
            <div class="max-w-6xl mb-12">
                <h1 class="text-4xl font-semibold text-gray-900 tracking-tight">Digital Contracts</h1>
            ...
            <div class="max-w-6xl space-y-6">
                <?php if (empty($contracts)): ?>
                    <div class="bg-white rounded-[3rem] border border-gray-100 p-20 text-center shadow-sm">
                        ...
                        <h3 class="text-2xl font-semibold text-gray-900">No contracts found</h3>
                        ...
                        <div class="bg-white rounded-[2.5rem] border border-gray-100 p-8 shadow-sm hover:shadow-2xl transition-all group relative overflow-hidden">
                            ...
                            <div class="flex flex-col md:flex-row md:items-center gap-10">
                                <div class="flex-1 flex items-center gap-8">
                                    ...
                                    <div>
                                        <div class="flex items-center gap-4 mb-2">
                                            <span class="text-[10px] font-semibold font-mono text-gray-400 tracking-widest uppercase">REF-<?= str_pad($contract['booking_id'], 5, '0', STR_PAD_LEFT) ?></span>
                                            <?php if ($contract['contract_status'] === 'signed'): ?>
                                                <span class="px-3 py-1 rounded-full bg-green-50 text-green-700 text-[10px] font-semibold uppercase tracking-widest shadow-sm shadow-green-200/50">Signed</span>
                                            <?php else: ?>
                                                <span class="px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-[10px] font-semibold uppercase tracking-widest shadow-sm shadow-amber-200/50 animate-pulse">Pending</span>
                                            <?php endif; ?>
                                        </div>
                                        <h3 class="text-2xl font-semibold text-gray-900 leading-tight mb-2"><?= htmlspecialchars($contract['brand'] . ' ' . $contract['model']) ?></h3>
                                        <div class="flex items-center gap-2 text-gray-400">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                            <span class="text-xs font-semibold uppercase tracking-tight"><?= date('M j, Y', strtotime($contract['pickup_date'])) ?> — <?= date('M j, Y', strtotime($contract['return_date'])) ?></span>
                                        </div>
                                    </div>
                                </div>
                                <div class="flex flex-row md:flex-col items-stretch gap-4 w-full md:w-56 pt-6 md:pt-0 border-t md:border-t-0 md:border-l border-gray-100 md:pl-10">
                                    <?php if ($contract['contract_status'] === 'signed'): ?>
                                        <a href="/api/download-contract.php?booking_id=<?= $contract['booking_id'] ?>" target="_blank" class="flex-1 flex items-center justify-center gap-2 px-8 py-5 bg-gray-900 text-white rounded-[1.5rem] text-xs font-semibold uppercase tracking-widest hover:bg-black shadow-2xl shadow-gray-200 transition-all active:scale-95">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                            PDF Copy
                                        </a>
                                    <?php else: ?>
                                        <a href="/templates/contract-sign.php?booking_id=<?= $contract['booking_id'] ?>&token=<?= htmlspecialchars($contract['signing_token']) ?>" class="flex-1 flex items-center justify-center gap-2 px-8 py-6 text-white rounded-[1.5rem] text-xs font-semibold uppercase tracking-[0.2em] shadow-2xl shadow-amber-200 transition-all active:scale-95 hover:opacity-90 animate-pulse" style="background: linear-gradient(135deg, #f59e0b, #d97706)">
                                            Sign Now
                                        </a>
                                    <?php endif; ?>
                                </div>
